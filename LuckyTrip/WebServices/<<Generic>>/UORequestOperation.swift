...
protocol UORequestOperationProtocol: NSObjectProtocol {
    ...
    func multipartRequestData() -> MultipartRequestData?
}
...
class UORequestOperation<T : Codable, P> : UOOperation<T,P>, UORequestOperationProtocol{
    ...
    func sendRequest( _ request : URLRequest){
        ...
        if cachingURL(){
            ...
            requestToSend.cachePolicy = .reloadIgnoringCacheData
        }
        print("\n body Request:\(params) \n")
...
//                            print(" \n Response result : \(String(data: data, encoding: .utf8)) \n")
    ...
    func uploadRequestMultipart( _ request:URLRequest){
        var requestToSend = request
        ...
        print("\n body Request upload image with params:\(params) \n")
        ...
        let fileSize = countBytes.string(fromByteCount: Int64(multipartRequestData()!.file.count))
        let fileCount = Float(multipartRequestData()!.file.count) / 1024.0 / 1024.0
        print("File size: \(fileSize)")

        print("File size/dd: \(fileCount)")
        if fileCount > 2.0
        {
            self.raisError(message: "The file size should be less than 2 MB, please try again".localized, code: -7300)
            return
        }
        do {
            requestToSend.httpBody = try createBody(with: params, multipart:  multipartRequestData()!, boundary: boundary)
        } catch let errorE{
            print(errorE.localizedDescription)
            self.raisError(message: errorE.localizedDescription, code: -7003)
            return
        }
        requestToSend.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        let task = URLSession.shared.dataTask(with: requestToSend) { [weak self] data, response, error in
            guard let wself = self else {return}
            if let error = error {
                wself.raisError(message:error.localizedDescription, code: (response as? HTTPURLResponse)?.statusCode ?? -7003)
            }else{
    ...
    private func createBody(with parameters: [String: Any]? = nil,multipart:MultipartRequestData, boundary: String) throws -> Data {
        var body = Data()
        ...
        body.append("--\(boundary)\r\n")
        body.append("Content-Disposition: form-data; name=\"\(multipart.keyName)\"; filename=\"\(multipart.fileName)\"\r\n")
        body.append("Content-Type: \(multipart.mimType)\r\n\r\n")
    ...
    func multipartRequestData() -> MultipartRequestData? {return nil}
}
